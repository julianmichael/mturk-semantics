...
object ValidationHITManager {
  ...
  def loadPrompts[SID : Reader] = loadDataFile(finalExperimentName, validationPromptsFilename)
    .toOption
...
class ValidationHITManager[SID : Reader : Writer] private (
  ...
  var allWorkerInfo = {
    loadDataFile(finalExperimentName, workerInfoFilename)
      .map(_.mkString)
  ...
  private[this] var promptToAssignments = {
    loadDataFile(finalExperimentName, promptToAssignmentsFilename)
      .map(_.mkString)
