...
val projectName = "turksem-interactive" // make sure it matches the SBT project;
// this is how the .js file is found to send to the server

val annotationPath = java.nio.file.Paths.get(s"data/interactive/$label/annotations")
...
def getPromptInflectedTokens(i: Int) =
  exp.genAjaxService.processRequest(IQAAjaxRequest(getId(i))).inflectedTokens
