...
class GenerationHITManager(
  ...
  var badSentences = loadDataFile(finalExperimentName, badSentenceIdsFilename)
    .map(_.mkString)
  ...
  private[this] def flagBadSentence(id: SentenceId) = {
    ...
    service.searchAllHITs.iterator
      ...
      .map(_.getHITId)
      .map(hitDataService.getHIT[GenerationPrompt](hitTypeId, _).get)
  ...
  var allWorkerStats =
    loadDataFile(finalExperimentName, workerStatsFilename)
      .map(_.mkString)
  ...
  var feedbacks =
    loadDataFile(finalExperimentName, feedbackFilename)
      .map(_.mkString)
  ...
  override lazy val receiveAux2: PartialFunction[Any, Unit] = {
    ...
    case ValidationResult(prompt, hitId, assignmentId, numQAsValid) =>
      val ha = for {
        hit <- hitDataService.getHIT[GenerationPrompt](hitTypeId, hitId).toOptionPrinting.toList
