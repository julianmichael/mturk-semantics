...
object FileManager {
  ...
  /** Loads all saved HITs and Assignments from disk for a HIT type.
    ...
    *
    * @param hitType the HIT Type ID to load all of the data for
  ...
  def loadAllData[Prompt: Reader, Response : Reader](
    ...
    val allData = for {
      ...
      assignments = loadAssignmentsForHIT[Response](hit.hitType, hit.hitId)
  ...
  /** Loads all approved assignments for the given HIT info.
    ...
    *
    * @param hitType the HIT Type ID of the HIT for which we want assignments
