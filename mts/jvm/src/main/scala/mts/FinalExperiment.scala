...
class FinalExperiment(implicit config: TaskConfig) {
      ...
      scala.util.Try(
        stats -> SentenceHITInfo(
          ...
          stats.genHITIds.toList
            .map(hitDataService.getHITInfo[GenerationPrompt, List[WordedQAPair]](genTaskSpec.hitTypeId, _))
            .map(_.get),
          stats.valHITIds.toList
            .map(hitDataService.getHITInfo[ValidationPrompt, List[ValidationAnswer]](valTaskSpec.hitTypeId, _))
            .map(_.get))
      ).toOption
    }.toMap
    SummaryInfo(
  ...
  lazy val oldGenInfos: List[HITInfo[GenerationPrompt, List[WordedQAPair]]] =
    (hitDataService.getAllHITInfo[GenerationPrompt, List[WordedQAPair]](oldGenHITTypeId).get
       ++ hitDataService.getAllHITInfo[GenerationPrompt, List[WordedQAPair]](oldGenHITTypeId2).get)
  lazy val oldValInfos: List[HITInfo[ValidationPrompt, List[ValidationAnswer]]] =
    (hitDataService.getAllHITInfo[ValidationPrompt, List[ValidationAnswer]](oldValHITTypeId).get
       ++ hitDataService.getAllHITInfo[ValidationPrompt, List[ValidationAnswer]](oldValHITTypeId2).get)

  lazy val allGenInfos: List[HITInfo[GenerationPrompt, List[WordedQAPair]]] =
    oldGenInfos ++ hitDataService.getAllHITInfo[GenerationPrompt, List[WordedQAPair]](genTaskSpec.hitTypeId).get
  lazy val allValInfos: List[HITInfo[ValidationPrompt, List[ValidationAnswer]]] = oldValInfos ++ hitDataService.getAllHITInfo[ValidationPrompt, List[ValidationAnswer]](valTaskSpec.hitTypeId).get
  ...
  def sampleQAs(shuffleRand: Random, proportionQAsToSample: Double = 0.2, sentenceToQAs: Map[SentenceId, List[SourcedQA]], n: Int) = {
