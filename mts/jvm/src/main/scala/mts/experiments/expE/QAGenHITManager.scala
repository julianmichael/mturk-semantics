...
class QAGenHITManager(
  taskSpec: TaskSpecification{ type Prompt = QAGenPrompt ; type Response = QAGenResponse },
  numAssignmentsPerPrompt: Int,
  numHITsToKeepActive: Int,
  _promptSource: Iterator[QAGenPrompt])(
  implicit pr: Reader[QAGenPrompt],
  config: TaskConfig
) extends HITManager[QAGenPrompt, QAGenResponse](taskSpec) {
